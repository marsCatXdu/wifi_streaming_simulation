...
| `packets_submitted`, `packets_remaining_to_submit` | `MultipathSender::SendPacket` and plan | F0 | packets | Cumulative and complementary | Never null; zero is meaningful | Application state |
| `application_socket_packet_bytes_submitted` | `Packet::GetSize()` before successful `Socket::Send()` | F0 | application socket packet bytes | Cumulative | Never null; zero is meaningful | Application state |
...
| `mpdu_tx_successes_total`, `last_tx_success_time_ns` | `AckedMpdu` after a pending tagged attempt | F1-ideal | count or ns | Cumulative; one success per stable MPDU | Null only without Wi-Fi binding; timestamp null before first success | Driver TX success counters |
...
| `frame_packets_currently_queued`, `frame_mac_service_bytes_currently_queued` | Tracked tagged entries in the selected target BE queue | F2 | packets, MAC service bytes | Instantaneous exact queue state | Null without queue support; zero is meaningful | Modified driver |
| `mac_queue_packets`, `mac_queue_service_bytes`, `mac_queue_oldest_enqueue_time_ns` | All tagged streaming entries in the selected path's target BE queue | F2 | packets, MAC service bytes, ns | Instantaneous exact target-traffic scope | Counts/bytes zero when empty; oldest timestamp null when empty | Modified driver |
| `packets_ahead_of_frame`, `mac_service_bytes_ahead_of_frame` | FIFO order of tracked target queue entries | F2 | packets, MAC service bytes | Instantaneous; older entries before the first queued packet of this frame, or all entries at T0 | Both null when exact ordering is unavailable; zero is meaningful | Modified driver |
...
| `current_cw`, `remaining_backoff_slots` | Selected `QosTxop::GetCw()` and `GetBackoffSlots()` | F3 | slots | Instantaneous and side-effect free | Null when oracle collection is disabled | Internal/modified driver |
...
| `expected_access_reason_within_slack` | `ChannelAccessManager::GetExpectedAccessWithin(deadline_slack)` | F3 | documented `WifiExpectedAccessReason` token | Instantaneous side-effect-free query; not a frame-specific access promise | Null when oracle collection is disabled | No commodity equivalent |
...
| `phy_tx_time_{window}_us`, `phy_rx_time_{window}_us`, `phy_busy_time_{window}_us`, `phy_idle_time_{window}_us`, `phy_other_time_{window}_us` | Clipped target `WifiPhyStateHelper::State` intervals | F1-ideal | us | Exact covered duration; OTHER is switching, sleep, and off |
...
unsupported member, such as ACK signal in Increment 1, into a fabricated zero.
...
IPv4/UDP MAC service bytes, rolling boundaries, full PHY coverage, oracle
nulls and vocabularies, event ordering, and canonical support masks. It
